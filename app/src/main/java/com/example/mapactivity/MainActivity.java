...
public class MainActivity extends AppCompatActivity {
    ...
    private final LatLng mDestinationLatIng = new LatLng(43.0753796, -89.4064015);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mapFragment.getMapAsync(googleMap -> {
            mMap = googleMap;
            googleMap.addMarker(new MarkerOptions().position(mDestinationLatIng).title("Bascom Hill"));
            displayMyLocation();
        });
        ...
        if(permission== PackageManager.PERMISSION_DENIED) {
            ...
            mFusedLocationProviderClient.getLastLocation().addOnCompleteListener(this, task -> {
                ...
                if(task.isSuccessful() && mLastKnownLocation != null) {
                    ...
                    mMap.addPolyline(new PolylineOptions().add(currLocation, mDestinationLatIng));
                }
            });
        }
    }
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        Log.i("oRPR", "right inside function");
        if (requestCode == PERMISSIONS_REQUEST_ACCESS_FINE_LOCATION) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                displayMyLocation();
                Log.i("oRPR", "displayLocation called");
            }
        }
    }
}